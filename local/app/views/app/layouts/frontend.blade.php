...
<head>
	...
	<link rel="stylesheet" href="{{ url('/assets/css/frontend.css?v=' . Config::get('version.version')) }}" />
	<link rel="stylesheet" href="{{ url('/assets/css/custom/frontend.general.css?v=' . Config::get('version.version')) }}" />
...
<body class="theme-default @yield('body_class')<?php if(\Lang::has('i18n.dir') && trans('i18n.dir') == 'rtl') echo ' right-to-left'; ?> {{ \App\Controller\HookController::hook('body_class'); }}" style="background-image: url('{{ str_replace('http://', '//', \App\Core\Settings::get('cms_bg_login', url('assets/images/bg/login.jpg'))); }}') !important">

@yield('content')

<script src="{{ url('/assets/js/frontend.js?v=' . Config::get('version.version')) }}"></script>
