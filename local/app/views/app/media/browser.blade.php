...
@section('content')
	...
	<div class="page-header">
		<h1 style="height:32px"><i class="ion-ios-folder-outline page-header-icon"></i> {{ trans('global.media_browser') }}</h1>
	</div>
...
<?php } else { ?>
            commands : [
              'rm'<?php if ($upload) { ?>
                ,/*'open', */'reload', 'home', 'up', 'back', 'forward', 
                'download', 'rename', 'mkdir', 'upload', 'copy', 
                'paste'/*, 'edit'*/, 'search', 'info', 'view',
                'resize', 'sort'<?php } ?>
            ],
<?php } ?>
