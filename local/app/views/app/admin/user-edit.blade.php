...
@section('content')
  ...
  <div class="page-header">
    <h1><i class="ion-person page-header-icon"></i> {{ trans('global.edit_user') }}</h1>
  </div>
<?php
echo Former::open()
...
echo Former::select('reseller_id')
  ->class('select2-required form-control')
  ->name('reseller_id')
  ->forceValue($user->reseller_id)
  ->fromQuery(\App\Model\Reseller::orderBy('domain')->get(), 'domain', 'id')
...
$reseller_disabled = ($reseller->id != 1) ? true : false;

echo Former::select('plan')
  ->class('select2-required form-control')
  ->name('plan')
  ->forceValue($user->plan_id)
 // ->fromQuery(\App\Model\Plan::where('reseller_id', $reseller->id)->orWhere('id', 1)->orderBy('sort')->get(), 'name', 'id')
  ->fromQuery(\App\Model\Plan::orderBy('sort')->get(), 'name', 'id')
  ->label(trans('global.plan'))
  ->disabled($reseller_disabled);
