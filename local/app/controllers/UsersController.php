...
class UsersController extends Controller
  ...
  function store()
  {
    ...
    $email_host = substr(strrchr($email, "@"), 1);

    // Check disposable email address
    $disposable_found = false;

    $mails = \File::get(app_path() . '/data/disposable-mail.txt');
    $mails = explode(PHP_EOL, $mails);

    foreach ($mails as $mail)
    {
      if ($email_host == trim($mail))
      {
    ...
    // Check free email address
    $free_found = false;
/*
    $mails = \File::get(app_path() . '/data/free-mail.txt');
    $mails = explode(PHP_EOL, $mails);

    foreach ($mails as $mail)
    {
      if (ends_with($email, trim($mail)))
      {
        $free_found = true;
        break;
      }
...
*/
    if ($disposable_found || $free_found)
    {
      return Redirect::action('UsersController@create')->withInput(Input::except('password'))->with('error', ['We don\'t allow free email addresses. Use your business or personal email.']);
      die();
    }
    else
    {
