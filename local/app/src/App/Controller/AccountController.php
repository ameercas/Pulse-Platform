...
class AccountController extends \BaseController {
  ...
  {
    if (\Config::get('social-login.member.app_redirect') != '')
    {
      session_start();

      $provider = new \League\OAuth2\Client\Provider\GenericProvider([
        'clientId'        => \Config::get('social-login.member.app_id'),
        'clientSecret'      => \Config::get('social-login.member.app_secret'),
        'redirectUri'       => \Config::get('social-login.member.app_redirect'),
        'urlAuthorize'      => \Config::get('social-login.member.app_host') . '/oauth/authorize',
        'urlAccessToken'      => \Config::get('social-login.member.app_host') . '/oauth/access_token',
        'urlResourceOwnerDetails' => \Config::get('social-login.member.app_host') . '/oauth/resource'
      ]);
      ...
      else
      {
        try {
          ...
          if (! $accessToken->hasExpired())
          {
            ...
            $member = $provider->getResourceOwner($accessToken);
            ...
            $user = \User::where('reseller_id', $reseller->id)->where('email', $member['email'])->first();

            if ($user != NULL)
            {
              // Check if user with provider
              $user = \User::where('reseller_id', $reseller->id)->where('provider', \Config::get('social-login.member.app_provider'))->where('email', $member['email'])->first();
              if ($user == NULL)
              {
            ...
            if ($user == NULL)
            {
              ...
              $user->parent_id = NULL;
              $user->username = substr($member['username'], 0, 16);
              $user->provider = \Config::get('social-login.member.app_provider');
              $user->plan_id = 1;
              ...
              $user->first_name = explode('@', $member['email'])[0];
              $user->last_name = '';
              $user->timezone = $member['timezone'];
  ...
  {
    if(\Config::get('app.demo', false) && Auth::user()->id == 1)
    {
      echo "<script>alert('You can\'t update the master profile avatar in demo mode.')</script>";
      die();
    }
  ...
  public function postSave()
  {
    ...
    if(\Config::get('app.demo', false) && Auth::user()->id == 1)
    {
      return \Response::json(array(
        'result' => 'error', 
        'result_msg' => 'You can\'t update the master profile in demo mode.'
      ));
    }
  ...
  public function postUserUpdate()
  {
    ...
    if(\Config::get('app.demo', false) && $sl['user_id'] == 1)
    {
      return \Response::json(array(
        'result' => 'error', 
        'result_msg' => 'You can\'t update the master profile in demo mode.'
  ...
  {
    if(Auth::user()->can('system_management') && Auth::user()->id == 1 && ! \Config::get('app.demo', false))
    {
