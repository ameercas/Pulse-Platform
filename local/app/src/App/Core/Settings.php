...
class Settings {
  ...
  public static function get($name, $default = NULL, $user_id = 0)
  {
    ...
    $reseller_id = (! $reseller) ? 0 : $reseller->id;

    //$return = \Cache::rememberForever('settings_' . $name . '_' . $reseller_id . '_' . $user_id, function() use($name, $default, $user_id, $reseller, $reseller_id)
    //{
      $oSetting = \App\Model\Setting::where('name', $name)->where('reseller_id', $reseller_id)->where('user_id', $user_id)->first();
  ...
  public static function set($name, $value, $user_id = 0)
  {
    ...
    //\Cache::forget('settings_' . $name . '_' . $reseller_id . '_' . $user_id);

    $oSetting = \App\Model\Setting::where('name', $name)->where('reseller_id', $reseller_id)->where('user_id', $user_id);
    ...
    elseif($value != NULL)
    {
      ...
      $oSetting->reseller_id = $reseller_id;
