...
*/
  ...
  public function showSite($local_domain)
  {
    ...
    // Get cookies
    //$continue_cookie_name = 'continue_without_app' . md5($local_domain);
    $continue_cookie_name = 'continue_without_app';
    $continue = (boolean) \Cookie::get($continue_cookie_name);

    // Check if reseller has app
    $reseller = \App\Controller\ResellerController::get();
    if ($reseller !== false && $reseller->app_name == '') $continue = true;
    ...
    if(! $edit && $ua->isMobile())
    {
      // Check if right app is used
      // Also check whether iOS or Android and show appropriate download link
      if (! $continue)
      {
        $continue = (boolean) \Request::get('continue', false);
        if ($continue)
        {
          $continue_without_app = \Cookie::make($continue_cookie_name, 1, 30); // Minutes
        }
      }
      ...
      if (! $lat && ! $lng && ! $continue)
      {
        ...
        switch ($os['name'])
        {
          case 'iOS': $os = 'apple'; $download_link = $reseller->app_link_ios; break;
          //case 'Android': $os = 'android'; $download_link = $reseller->app_link_android; break;
          default: $os = 'android'; $download_link = $reseller->app_link_android;
        }

        if ($download_link != '')
        {
          return View::make('user.app.tpl-download-app', [
    ...
    if (isset($continue_without_app))
    {
      $view = View::make('user.site.main', array(
        'preview' => false,
        'published' => $published,
        'edit' => $edit,
      ...
      return \Response::make($view)->withCookie($continue_without_app);
    }
    else
    {
      return View::make('user.site.main', array(
  ...
  public function postNew()
  {
    ...
    else
    {
      $response = array(
        'result' => 'error', 
        'result_msg' => $homepage->errors()->first()
      );
    }
  ...
  public function postSiteSettings()
  {
    ...
    if(count($site) > 0)
    {
      ...
      $rules = array(
        'local_domain'  => 'required|min:3|max:42|regex:/^[a-zA-Z0-9_-]+$/|unique:apps,local_domain|unique:sites,local_domain,' . $qs['site_id']
      );
      ...
      // Prevent same domain or reseller update
      $reseller = \App\Model\Reseller::where('domain', $domain)
        ->orWhere('domain', 'www.' . $domain)
        ->first();

      if(count($reseller) == 0) $site->domain = $domain;
